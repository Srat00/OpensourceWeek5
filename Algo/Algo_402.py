...
from Algo_401 import FixedStack
...
while True:
    menu = selectMenu()
    if menu == Menu.Push:
        value = int(input('집어넣을 데이터를 입력하세요 >>  '))
        stack.push(value)
        
    elif menu == Menu.Pop:
        value = stack.pop()
        print('꺼낸 데이터는 {}입니다.'.format(value))
        
    elif menu == Menu.Peek:
        value = stack.peek()
        print('가장 위쪽 데이터는 {}입니다.'.format(value))
        ...
        if result == 0:
            print('스택에 {}은(는) 없습니다.'.format(value))
        else:
            print('스택에 있는 {}은(는) {}개입니다.'.format(value, result))
